      ...
      <form @submit.prevent="handleSubmit(onSubmit)">
        <b-row>
                        ...
                        <b-row align-v="center">
                          ...
                          <b-form-group class="col-md-12" label="Password" label-for="username">
                            <ValidationProvider name="Password">
                              <b-form-input v-model="contractor.password" type="password" placeholder="Password"></b-form-input>
                              <span>Kosongkan password jika tidak ingin dirubah</span>
                            </ValidationProvider>
                          </b-form-group>
                        </b-row>
                    </template>
          ...
          <b-col lg="12">
              <template>
                <div class="iq-edit-list">
                    <b-button variant="primary" type="submit" class="mb-3 float-right mr-1 btn-lg w-25"><i class="ri-bill-fill"></i>Simpan</b-button>
...
export default {
  ...
  methods: {
    ...
    getDataContractor () {
      API.contractor
        .getDetailContractor(this.$route.params.id).then(response => {
          this.contractor = response.data.result
        }).finally(() => { this.loading = false })
    },
    addContractorData () {
      API.contractor
        .editContractor(this.$route.params.id, this.contractor).then(response => {
          if (response.status) {
